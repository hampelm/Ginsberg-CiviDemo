...
<div id="notes">
    ...
    {/literal}
        ...
        {foreach from=$notes item=note}
            ...
            <div id="cnote_{$note.id}" class="{cycle values="odd-row,even-row"} crm-note">
                <h2>{$note.subject}</h2>
            
                <p class="byline"><a href="{crmURL p='civicrm/contact/view' q="reset=1&cid=`$note.contact_id`"}">{$note.createdBy}</a> on {$note.modified_date|crmDate}</p>
                ...
                {$note.action|replace:'xx':$note.id}
