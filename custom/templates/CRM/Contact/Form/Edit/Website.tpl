...
{if !$addBlock}
<tr>
    <td><label>{ts}Website{/ts}
        
        <a href="#" title={ts}Add{/ts} onClick="buildAdditionalBlocks( 'Website', '{$className}');return false;">{ts}add{/ts}</a></label>
    </td>
    <td colspan="2"></td>
...
<tr id="Website_Block_{$blockId}">
    <td colspan="2">{$form.website.$blockId.url.html|crmReplace:class:twenty}&nbsp;{$form.website.$blockId.website_type_id.html}</td>
