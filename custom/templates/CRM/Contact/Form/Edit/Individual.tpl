     ...
     if (!isNaN(cid) || ! checkSimilar)
       return;//no dupe check if this is a modif or if checkSimilar is disabled (CIVICRM_CONTACT_AJAX_CHECK_SIMILAR in civicrm_setting)
         ...
         function(data){
           ...
           var msg="<tr id='lastname_msg'><div colspan='5'><div class='messages status'><div class='icon inform-icon'></div>";
           ...
           cj.each(data, function(i,contact){
             msg = msg + '<tr><div><a href="'+viewIndividual+contact.contact_id+'">'+ contact.display_name +'</a></div><div>'+contact.email+'</div><div class="action-items"><a class="action-item action-item-first" href="'+viewIndividual+contact.contact_id+'">{/literal}{ts}View{/ts}{literal}</a><a class="action-item" href="'+editIndividual+contact.contact_id+'">{/literal}{ts}Edit{/ts}{literal}</a></div></tr>';
           });
           msg = msg+ '</table>';
           cj('#last_name').parent().parent().after(msg+'</div><div></tr>');
...
-->
        <div id="form-first-name" class="form-item">
        ...
        <div class="form-item">
            ...
            {$form.job_title.html} at {$form.current_employer.html}
        </div>
        ...
        <hr />
            ...
            <!--
                    <div id="form-last-name" class="form-item">
                        ...
                        <div id="form-middle-name" class="form-item">
                        ...
                        <div id="form-employer" class="form-item">

                            {$form.current_employer.label}
                            <span id="employer_address" style="display:none;"></span>
                        </div>
